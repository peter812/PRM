...
-   **Anonymize People:** The graph page includes a toggle to hide names on all people nodes except the "Me" node. This feature is useful for taking screenshots or sharing the graph visualization while preserving privacy of contacts. When enabled, all person names are hidden except for the current user's node.
