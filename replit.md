...
-   **Global Search:** Real-time top-bar search with prioritized results for people and groups. The search-settings popup (category order/visibility) opens from a 3-dot icon embedded inside the search bar at its right edge.
-   **Contextual Top Bar (Apr 2026):** The top bar has three segments — left: sidebar trigger plus a contextual page title derived from the current route via `getPageTitle` in `client/src/lib/page-title.ts` (e.g. "People", "Social Accounts", "Link Accounts"); middle: the global search bar (with the 3-dot search-settings trigger inside it); right: a reserved contextual-actions slot (currently empty, intended for per-page controls in future tasks). Settings, theme toggle, and logout are no longer in the top bar — they live in the sidebar footer (`SidebarFooter` in `app-sidebar.tsx`) on both desktop and mobile, and the footer items show tooltips when the sidebar is collapsed to icon-only mode.
...
-   **Tasks Settings Page (Feb 2026):** Settings page at `/settings/tasks` for managing background tasks. Features: card with available tasks (single account follower count refresh with searchable Popover+Command account selector requiring 3+ characters, mass refresh), mass refresh button at the top, live-updating task history list that polls every 3 seconds only when tasks are pending/running. Red trash bin button on pending/running tasks to cancel them. Task cancellation sets status to "cancelled" and the task worker checks cancellation state during long-running operations (e.g., mass refresh) to abort early. API endpoints: GET `/api/tasks`, GET `/api/tasks/:id`, GET `/api/tasks/social-accounts-brief`, POST `/api/tasks/refresh-follower-count/:socialAccountId`, POST `/api/tasks/mass-refresh-follower-count`, DELETE `/api/tasks/:id` (cancel).
-   **Historical Social Account Tracking (Feb 2026):** Social accounts now use a four-table model:
    ...
    - API endpoints: GET `/api/social-accounts/:id/network-state`, POST `/api/social-accounts/:id/network-state`.
-   **Social Account History (Aug 2026):** `social_account_history` is a reverse-delta journal — `social_accounts` holds the current profile and counts, and each journal row records only what changed to get there (followers/following added and lost, plus the *previous* nickname, bio, location, and image). Entry kinds are `direct` (this account was scraped), `neighbour` (it turned up inside another account's scrape) and `baseline` (synthetic, written by the migration). `profileFieldsChanged`, not a null check, is what says whether a field changed. The `delta` column holds the account ids behind the counts and is read only by the detail endpoint. API endpoints: GET `/api/social-accounts/:id/history` (`kind`, `page`, `limit`), GET `/api/social-accounts/:id/history/summary`, GET `/api/social-accounts/history/:entryId` (`listLimit`, `listOffset`). Surfaced by the History tab on the social account profile. Replaces GET `/api/social-accounts/:id/profile-versions` and GET `/api/social-accounts/:id/network-changes`.
