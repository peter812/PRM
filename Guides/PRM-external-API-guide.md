...
PRM integrates an external [Qdrant](https://qdrant.tech) vector database paired with local [Ollama](https://ollama.com) embeddings to power semantic Super Search, daily journal retrieval, and AI App Knowledge Base RAG.

### Get Vector Storage Settings
...
```http
POST /api/vector/settings
Content-Type: application/json

{
  "enabled": true,
  "qdrantUrl": "http://localhost:6333",
  "qdrantApiKey": "optional-secret-key",
  "collectionName": "prm_daily_notes",
  "embeddingModel": "nomic-embed-text:latest"
}
```
...
Verifies network connectivity and collection availability against the configured Qdrant server.
...
{
  "ok": true,
  "message": "Connected to Qdrant successfully. Collection \"prm_daily_notes\" exists."
}
```

---

### Get Daily Notes Vector Stats
...
```json
{
  ...
  "vectorized": 42,
  "missing": 0,
  "lastSyncedAt": "2026-06-28T14:20:00.000Z"
}
```
...
Triggers a background batch embedding job for all daily notes missing vectors.

```http
POST /api/daily-notes/vectorize-all
```
...
Queries the local Ollama instance for models equipped for text embedding generation.

```http
GET /api/vector/embedding-models
```
...
```json
{
  "models": [
    ...
    { "name": "mxbai-embed-large:latest", "parameterSize": "335M" }
  ]
}
```
...
### Universal Vectorization (Super Search)

Universal vectorization embeds all primary PRM entities into a unified Qdrant collection (`prm_universal`), including `person`, `group`, `note`, `interaction`, `social_account`, `daily_note`, `image`, `ai_chat`, and `message`.

#### Get Universal Settings & Status

```http
GET /api/vector/universal/settings
GET /api/vector/universal/status
GET /api/vector/universal/stats
```
...
```json
{
  "person": { "total": 120, "vectorized": 120 },
  "group": { "total": 14, "vectorized": 14 },
  "note": { "total": 85, "vectorized": 85 },
  "interaction": { "total": 92, "vectorized": 92 },
  "social_account": { "total": 60, "vectorized": 60 },
  "daily_note": { "total": 42, "vectorized": 42 },
  "image": { "total": 18, "vectorized": 18 }
}
```

---

#### Trigger Universal Vectorize All

```http
POST /api/vector/universal/vectorize-all
```
...
Query semantic concepts across heterogeneous entities in a single ranked request.

```http
GET /api/vector/universal/search?q=machine+learning+researcher&limit=10&type=person
```

**Query Parameters:**
- `q` (required): Free-form semantic search query.
- `type` (optional): Filter to a specific entity type (`person`, `group`, `note`, `interaction`, `social_account`, `daily_note`, `image`, `ai_chat`, `message`).
- `limit` (optional): Maximum results (default: 20, max: 100).
- `threshold` (optional): Minimum cosine similarity score (default: 0.70).
  ...
  "results": [
    {
      ...
      "type": "person",
      "title": "Dr. Sarah Lin",
      "snippet": "Principal AI Scientist at DeepTech Labs. Tags: artificial-intelligence, graph-neural-networks. Email: sarah.lin@example.com",
      "score": 0.884,
...
### App Knowledge Base (RAG)

Indexes the `prm-app-knowledge.csv` documentation into Qdrant (`prm-app-knowledge` collection) for retrieval-augmented generation in chat.

#### Search App Knowledge

```http
GET /api/vector/app-knowledge/search?q=how+to+export+database+xml&limit=3
```
  ...
  "results": [
    {
      ...
      "content": "Export All Data downloads your entire PRM database as XML...",
      "score": 0.912
    }
  ]
}
...
```http
POST /api/vector/app-knowledge/reindex
```
...
PRM incorporates an Ollama-powered conversational chat engine with multi-step agentic function-calling tools and human-in-the-loop authorization.

### Verify Ollama Backend
...
{
  "ok": true,
  "models": ["llama3.2:latest", "mistral:latest", "nomic-embed-text:latest"]
}
```

---

### Streaming Agentic Chat (Server-Sent Events)

Initiate a streaming conversational response that can execute multi-step tool calls.

```http
GET /api/ai/chat/stream?message=When+did+I+last+meet+with+John+Doe?
```

**SSE Events Emitted:**
- `chunk`: Text token streaming from Ollama.
- `thought`: Internal chain-of-thought reasoning from the model.
- `tool_call`: Function tool invocation with JSON arguments (e.g. `get_person_interactions`).
- `tool_approval_request`: Emitted when a mutating (`write: true`) tool requires user confirmation in `auth` mode.
- `done`: Final generation metadata and token usage.

---

### Tool Approvals

When execution mode is `auth`, mutating tools pause and await resolution:

```http
POST /api/ai-tools/approvals/:id
Content-Type: application/json
...
Transcribe voice notes or voice recordings using the integrated Whisper microservice.
...
```json
{
  "text": "Had coffee with Alex today. We talked about launching the new marketing sprint next Tuesday."
}
```

---

### AI Image Vision Description

Generate descriptive captions and semantic labels for uploaded photos using vision models.
...
```json
{
  "description": "A group photo of four colleagues smiling outdoors in front of a modern office pavilion.",
  "labels": ["people", "outdoor", "business casual", "daytime"]
}
```
...
Track Instagram stories, automated profile snapshots, and network follower metrics.

### Get Account Stories

```http
GET /api/social-accounts/:id/stories?activeOnly=true
```
...
[
  {
    ...
    "mediaUrl": "/uploads/stories/story_9812.jpg",
    "mediaType": "image",
...
### Get Social Profile Version History

Retrieve historical snapshot changes of bio, nickname, follower count, and avatar URLs over time.

```http
GET /api/social-accounts/:id/history
```
...
[
  {
    ...
    "socialAccountId": "account-uuid",
    "nickname": "John Doe",
    "bio": "Updated bio text for summer 2026",
    "followersCount": 1840,
    "followingCount": 420,
...
Configure automated recurring profile scrapes and status audits:
...
## Facial Recognition & PRM-Face

Integrates with the external **PRM-Face** computer vision microservice for facial detection, S3 cutout storage, and identity clustering.
...
Pairwise contact rating game allowing comparative ranking across relationships.

### Get Random Voting Pair
...
Manage genealogical family trees, parents, children, and spouses.
...
```json
{
  ...
  "parents": [
    { "id": "uuid-mom", "firstName": "Mary", "lastName": "Doe", "relationshipId": "rel-1" }
  ],
  "children": [
    { "id": "uuid-child", "firstName": "Tim", "lastName": "Doe", "relationshipId": "rel-2" }
  ],
  "spouses": [
    { "id": "uuid-spouse", "firstName": "Anna", "lastName": "Doe", "isCurrent": true }
  ]
}
```
...
### Create Partnership Link

```http
POST /api/family/partnerships
Content-Type: application/json
...
API Version: 2.5
Last Updated: September 2026 (Expanded with Qdrant Vector Storage & Super Search, AI Agent Tools, Whisper STT, Stories & Tracking, PRM-Face, and Family Tree APIs)
