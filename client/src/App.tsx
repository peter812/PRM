...
function AppLayout() {
  ...
  // Track the last non-settings page so we can navigate back to it from settings
  const lastNonSettingsPage = useRef("/people");
  useEffect(() => {
    if (!isSettingsPage && !isAuthPage && !isWelcomePage) {
      lastNonSettingsPage.current = location;
    }
                  ...
                  <div className="hidden md:flex items-center gap-1">
                    {isSettingsPage ? (
                      <Button
                        ...
                        size="icon"
                        className="h-8 w-8"
                        onClick={() => navigate(lastNonSettingsPage.current)}
