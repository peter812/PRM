...
function participantName(p: any): string | null {
  if (p.person) return `${p.person.firstName} ${p.person.lastName}`;
  if (p.socialAccount) return p.socialAccount.username;
  return null;
}

// Name a scoped conversation after who the subject is talking TO — every
// participant except the subject themself. Perspective mode (other people's
// profiles) additionally treats the PRM owner, who has no person/account
// reference on their participant row, as a named counterpart.
function conversationDisplayName(conv: any, isSubject?: (p: any) => boolean, perspective = false): string {
  const others = (conv.participants || []).filter((p: any) => !isSubject?.(p));

  if (perspective) {
    const names = others.map((p: any) => participantName(p) ?? conv.metadata?.ownerName ?? "You");
    if (names.length > 0) return Array.from(new Set(names)).join(", ");
    return conv.metadata?.ownerName || "You";
  }

  if (conv.title) return conv.title;
  const names = others.map(participantName).filter(Boolean);
  return Array.from(new Set(names)).join(", ") || "Conversation";
}
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 px-6 text-center text-muted-foreground">
            ...
            {onImport && (
              <Button variant="outline" size="sm" className="rounded-full text-xs mt-4" onClick={onImport}>
          ...
          <div className="px-2 py-1">
            {conversations.map((conv: any) => {
              const name = conversationDisplayName(conv, isScoped ? isSubjectParticipant : undefined, perspective);
              ...
              const isFromSelf = perspective
                ...
                : conv.lastMessage && isSelfMessage(conv.lastMessage);
