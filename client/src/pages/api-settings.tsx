...
export default function ApiSettingsPage() {
  ...
  const createKeyMutation = useMutation({
    mutationFn: async ({ name }: { name: string }) => {
      const response = await apiRequest("POST", "/api/api-keys", { name, keyType: "full" });
      if (!response.ok) {
  ...
  return (
    <div className="container max-w-full py-3 md:py-8 px-4 md:pl-12 space-y-4 md:space-y-8 mx-auto md:mx-0">
      <div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <Badge variant="secondary" data-testid={`badge-key-type-${key.id}`}>
                          API Key
                        </Badge>
                      </div>
